...
class Porto_Importacao_Model extends CI_Model implements Interface_Porto {
	...
	public function findById( Porto $porto, $hub = FALSE )
	{
		...
		if( strtoupper($hub) == "DESTINO" )
		{
			...
			if( $rs->num_rows() < 1 )
			{
				log_message('error','Não foi possivel encontrar o porto de destino com o id informado');
				throw new Exception("Não foi possivel encontrar o porto de destino com o id informado");
			}
		...
		else
		{
			...
			if( $rs->num_rows() < 1 )
			{
				log_message('error','Não foi possivel encontrar o porto com o id informado');
				throw new Exception("Não foi possivel encontrar o porto com o id informado");
			}
